        ...
        ordered-section [
          {
            focus-ring = borderish {
              ...
              description = ''
                ...
                The focused window of the currently focused monitor, i.e. the window that can receive keyboard input, will be drawn according to ${link-opt (subopts (subopts parent).focus-ring).active}, and the last focused window on all other monitors will be drawn according to ${link-opt (subopts (subopts parent).focus-ring).inactive}.

                If you have ${link-opt (subopts parent).border} enabled, the focus ring will be drawn around (and under) the border.
              '';
            };
            ...
            border = borderish {
              ...
              description = ''
                ...
                The currently focused window, i.e. the window that can receive keyboard input, will be drawn according to ${link-opt (subopts (subopts parent).border).active}, and all other windows will be drawn according to ${link-opt (subopts (subopts parent).border).inactive}.

                If you have ${link-opt (subopts parent).focus-ring} enabled, the border will be drawn inside (and over) the focus ring.
              '';
            };
          }
          {
              ...
              let
                self = docs-only (decoration (self // { loc = [ "<decoration>" ]; })) // {
                  ...
                  description = ''
                    ...
                    This type specifically represents decorations drawn by niri: that is, ${link-opt (subopts parent).focus-ring} and/or ${link-opt (subopts parent).border}.
                  '';
                };
              in
              self;
          ...
          }
          {
            preset-column-widths = list preset-width // {
              description = ''
                ...
                ${fmt.nix-code-block ''
                  {
                    ${(subopts parent).preset-column-widths} = [
                      { proportion = 1. / 3.; }
                      { proportion = 1. / 2.; }
                      { proportion = 2. / 3.; }
            ...
            preset-window-heights = list preset-height // {
              description = ''
                ...
                ${fmt.nix-code-block ''
                  {
                    ${(subopts parent).preset-window-heights} = [
                      { proportion = 1. / 3.; }
                      { proportion = 1. / 2.; }
                      { proportion = 2. / 3.; }
          ...
          }
          {
            default-column-width = optional default-width { } // {
              description = ''
                ...
                See ${link-opt (subopts parent).preset-column-widths} for more information.
        ...
        imports = make-ordered-options [
...

          {
            workspaces =
              attrs-record (key: {
                ...
                layout = layout options (subopts options.workspaces).layout // {
...

          {
            outputs = attrs-record (key: {
              ...
              layout = layout options (subopts options.outputs).layout // {
          ...
          {
            layout = layout options options.layout;
          }
